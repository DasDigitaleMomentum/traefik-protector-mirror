...
func evaluatePrefilter(cfg *Config, req *http.Request) (prefilterDecision, error) {
	...
	rules := cfg.PrefilterRules
	requestURI := req.URL.RequestURI()
	if rules.URILengthMax > 0 && len(requestURI) > rules.URILengthMax {
		return prefilterDecision{Matched: true, Rule: "uri_length_max", Reason: "request URI length exceeds configured maximum"}, nil
	}

	rawQuery := req.URL.RawQuery
	if rules.QueryLengthMax > 0 && len(rawQuery) > rules.QueryLengthMax {
		return prefilterDecision{Matched: true, Rule: "query_length_max", Reason: "query length exceeds configured maximum"}, nil
	}
	...
	if rules.QueryParamCountMax > 0 {
		...
		if count > rules.QueryParamCountMax {
			return prefilterDecision{Matched: true, Rule: "query_param_count_max", Reason: "query parameter count exceeds configured maximum"}, nil
		}
	}
		...
		for key, values := range req.Header {
			for _, value := range values {
				if len(value) > rules.HeaderValueLengthMax {
					return prefilterDecision{Matched: true, Rule: "header_value_length_max", Reason: fmt.Sprintf("header %s exceeds configured maximum", key)}, nil
				}
			}
		}
	}
	...
	for _, prefix := range rules.DeniedPathPrefixes {
		...
		if strings.HasPrefix(pathLower, normalized) {
			return prefilterDecision{Matched: true, Rule: "denied_path_prefix", Reason: fmt.Sprintf("path matched denied prefix %s", normalized)}, nil
		}
	}
